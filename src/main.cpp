...
int main() {
  ...
  while(1) {
    ...
    motor_control1(motorSpeed.motor1SpeedIn, 0);

    motor_control2(motorSpeed.motor2SpeedIn, 0);

    motor_control3(motorSpeed.motor3SpeedIn, 0);

    motor_control4(motorSpeed.motor4SpeedIn, 0);

    serial.printf("pulse:%lu\n",pulse1);

    serial.printf("Motor speed: %lu\n", rpm1);
    serial.printf("string is %s\n", rx_buffer);
    serial.printf("Count is %d\n", countt);
    serial.printf("Newline Count is %d\n", newLineCount);
    serial.printf("Speed m1: %lu speed m2: %lu  speed m3: %lu speed m4: %lu\n", motorSpeed.motor1SpeedIn, motorSpeed.motor2SpeedIn, motorSpeed.motor3SpeedIn, motorSpeed.motor4SpeedIn);
...
void speed_string_parse(char* buffer, struct motorSpeed_t* motorSpeed)
{
  ...
  if(buffer[0] != '$' && buffer[strlen(buffer)] != ',') {
    serial.printf("Error received string!!\n");
    goto exit;
  }
